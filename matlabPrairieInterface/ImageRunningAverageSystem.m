classdef ImageRunningAverageSystem < matlab.System
    ...
    methods
        % Constructor
        function obj = ImageRunningAverageSystem(varargin)
    ...
    methods (Access = protected)
        ...
        function [update] = stepImpl(obj, image)
            % Implement algorithm. Calculate y as a function of
            % input u and discrete states.
            imageSize = size(image);  %#ok<*PROPLC>
            if ~isequal(imageSize, obj.imageSize)
            ...
            if isequal(image, lastFrame)
                % don't update the buffer if this is the same data as last
                % time
                update = false;
                return;
            end
