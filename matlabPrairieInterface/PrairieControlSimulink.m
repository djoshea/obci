classdef PrairieControlSimulink < handle
    % @djoshea 2017
    ...
    properties(SetAccess=protected)
        ...
        liveDisplayAverageImage
        
        simulinkRunningAverageEventListener
    end
    ...
    methods(Static)
        function inst = getInstance()
            inst = PrairieControlSimulink();
        end 
    end
    
    methods % UDP utils, send microscope bus, receive commands
        function pc = PrairieControlSimulink()
            try
                fromBase = evalin('base', 'PrairieControlSimulinkInstance');
                if isvalid(fromBase)
                    pc = fromBase;
                    return
                end
            ...
            PrairieControlSimulink.print('Connecting to PrairieLink\n');
            try
            ...
            res = pc.pl.Connect();
            if ~res
                error('Could not connect to PrairieLink');
            end
            
            assignin('base', 'PrairieControlSimulinkInstance', pc);
            
            % listen on udp
            PrairieControlSimulink.print('Connecting to UDP receive\n');
            ...
            PrairieControlSimulink.print('Connecting to UDP send\n');
...
%             readasync(pc.sockRecv);
    ...
    methods
        function delete(pc)
            if ~isempty(pc.pl)
                PrairieControlSimulink.print('Disconnecting PrairieLink\n');
                try
                    pc.pl.Disconnect();
                catch
                end
            ...
            if ~isempty(pc.sockRecv)
                PrairieControlSimulink.print('Disconnecting UDP\n');
                try
                    fclose(pc.sockRecv);
                catch
                end
        ...
        function str = udpReceive(pc, varargin)
            ...
            if ~isempty(str)
                %PrairieControlSimulink.print('Received commands %s\n', str);
            end 
        end
...
%                        if ~pc.isImaging
 ...
 %                       end
        ...
        function run(pc)
            PrairieControlSimulink.print('Waiting for UDP commands...\n');
            pc.udpFlush();
            while(true)
                str = pc.udpReceive();
                if ~isempty(str)
    ...
    methods % 2P imaging and file locations
        ...
        function setFileLocationTask(pc)
            ...
            PrairieControlSimulink.mkdirRecursive(folder);
            ...
            file = sprintf('Tseries_%s_%s_%s', dateStrNoHyphen, pc.subject, pc.protocol);
            assert(~any(isspace(file)), 'File cannot have spaces');
...
%             pc.setFileLocationTask();
    ...
    methods(Static)
        function print(varargin)
            fprintf('PrairieControlSimulink: ');
            fprintf(varargin{:});
        end
        ...
        function mkdirRecursive(dirPath)
            ...
            if exist(dirPath, 'dir')
                ...
                if ~isempty(parent)
                    PrairieControlSimulink.mkdirRecursive(parent);
                end
                mkdir(dirPath);
            end
        end
    ...
    methods
        ...
        function img = getImage(pc)
            ...
            img = uint16(pc.pl.GetImage_2(ch, y, x)); % dimensions follow matrix conventions here
        end
        ...
        function startLiveDisplay(pc)
            ...
            set_param(pc.modelName, 'StartFcn','PrairieControlSimulink.addEventListenersOnStart');
            
            % set the stop time to inf
            set_param(pc.modelName,'StopTime','inf');
            % set the simulation mode to normal
            set_param(pc.modelName,'SimulationMode','normal');

            % update the diagram
            set_param(pc.modelName, 'SimulationCommand', 'update');
            
            pc.liveDisplayActive = true;
            
            % start the model
            set_param(pc.modelName,'SimulationCommand','start');
        end
        
        function stopLiveDisplay(pc)
            pc.liveDisplayActive = false;
            set_param(pc.modelName,'SimulationCommand','stop');
        end
    ...
    methods(Static) % simulink support
        function addEventListenersOnStart()
            pc = PrairieControlSimulink.getInstance();
            pc.simulinkRunningAverageEventListener = add_exec_event_listener(...
                'LiveDisplay/RunningAverage', 'PostOutputs', ...
                @PrairieControlSimulink.simulinkRunningAverageEvent);
        end
        
        function simulinkRunningAverageEvent(block, eventdata)
            pc = PrairieControlSimulink.getInstance();
            now = toc();
            persistent lastUpdate
            if isempty(lastUpdate) || lastUpdate > now
                tic();
                lastUpdate = -Inf;
            end
            ...
            if now - lastUpdate > 0.2
                lastUpdate = now;
                pc.liveDisplayAverageImage = block.OutputPort(1).Data;
                pc.updateImage(pc.liveDisplayAverageImage);
            end
        end
    end
