function [out, valid] = serializeBusWithDataLoggerHeader_ToHostBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('ToHostBus')
    ...
    coder.varsize('out', 9193 + 174*numel(namePrefix) + headerLength);
    outSize = headerLength + getSerializedBusLength_ToHostBus(bus, namePrefix);
    assert(outSize <= headerLength + 9193 + 174*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(4178724571), uint16(163), timestamp);
