function [out, valid] = serializeBusWithDataLoggerHeader_PolarisInputBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('PolarisInputBus')
    ...
    outSize = headerLength + getSerializedBusLength_PolarisInputBus(bus, namePrefix);
    assert(outSize <= headerLength + 346 + 12*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(2787318375), uint16(12), timestamp);
