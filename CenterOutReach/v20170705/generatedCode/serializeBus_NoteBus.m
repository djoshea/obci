function [out, valid] = serializeBus_NoteBus(bus, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('NoteBus')
    ...
    valid = uint8(0);
    coder.varsize('out', 50055 + 2*numel(namePrefix));
    outSize = getSerializedBusLength_NoteBus(bus, namePrefix);
    assert(outSize <= 50055 + 2*numel(namePrefix));
