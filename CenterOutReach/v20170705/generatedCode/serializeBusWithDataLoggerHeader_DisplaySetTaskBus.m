function [out, valid] = serializeBusWithDataLoggerHeader_DisplaySetTaskBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('DisplaySetTaskBus')
    ...
    outSize = headerLength + getSerializedBusLength_DisplaySetTaskBus(bus, namePrefix);
    assert(outSize <= headerLength + 65 + 2*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(2304494671), uint16(2), timestamp);
