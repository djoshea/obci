function [bus, valid, offset] = deserializeBusForMatlab_AnalogInputBus(input, offset, valid, namePrefix)
%#codegen
% DO NOT EDIT: Auto-generated by 
%   BusSerialize.writeDeserializeBusCode('AnalogInputBus')
    ...
    bus = initializeBus_AnalogInputBus();
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        if valid && elements == uint32(0)
            ...
            assert(elements <= uint32(1));
            bus.photobox = zeros([1 1], 'uint8');
            bus.photobox = logical(bus.photobox);
            if elements > uint32(0)
                bus.photobox = logical(in(offset:offset+uint32(elements*1 - 1)));
