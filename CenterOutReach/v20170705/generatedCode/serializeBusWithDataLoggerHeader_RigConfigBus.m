function [out, valid] = serializeBusWithDataLoggerHeader_RigConfigBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('RigConfigBus')
    ...
    coder.varsize('out', 457 + 10*numel(namePrefix) + headerLength);
    outSize = headerLength + getSerializedBusLength_RigConfigBus(bus, namePrefix);
    assert(outSize <= headerLength + 457 + 10*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(1272042977), uint16(10), timestamp);
