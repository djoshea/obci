function [out, valid] = serializeBusWithDataLoggerHeader_TaskControlBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('TaskControlBus')
    ...
    outSize = headerLength + getSerializedBusLength_TaskControlBus(bus, namePrefix);
    assert(outSize <= headerLength + 252 + 10*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(3192905113), uint16(10), timestamp);
