function [out, valid] = serializeBusWithDataLoggerHeader_ScopeTimingBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('ScopeTimingBus')
    ...
    outSize = headerLength + getSerializedBusLength_ScopeTimingBus(bus, namePrefix);
    assert(outSize <= headerLength + 302 + 9*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(3855272733), uint16(9), timestamp);
