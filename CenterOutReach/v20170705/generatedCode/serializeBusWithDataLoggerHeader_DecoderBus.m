function [out, valid] = serializeBusWithDataLoggerHeader_DecoderBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('DecoderBus')
    ...
    coder.varsize('out', 432 + 4*numel(namePrefix) + headerLength);
    outSize = headerLength + getSerializedBusLength_DecoderBus(bus, namePrefix);
    assert(outSize <= headerLength + 432 + 4*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(2747312549), uint16(3), timestamp);
