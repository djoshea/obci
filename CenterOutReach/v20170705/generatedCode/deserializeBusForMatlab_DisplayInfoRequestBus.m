function [bus, valid, offset] = deserializeBusForMatlab_DisplayInfoRequestBus(input, offset, valid, namePrefix)
%#codegen
% DO NOT EDIT: Auto-generated by 
%   BusSerialize.writeDeserializeBusCode('DisplayInfoRequestBus')
    ...
    bus = initializeBus_DisplayInfoRequestBus();
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Deserializing fixed-sized field infoPoll
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    expectedHeader_infoPoll = uint8([2, 4, typecast(uint16(numel(namePrefix) + 8), 'uint8'), namePrefix, 'infoPoll', typecast(uint16(4), 'uint8'), 'bool', 9, 1])';
    for headerOffset = 1:uint32(20+numel(namePrefix)-1)
        valid = uint8(valid && in(offset+headerOffset-1) == expectedHeader_infoPoll(headerOffset));
    end
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        if valid && elements == uint32(0)
            ...
            bus.infoPoll = zeros([1 1], 'uint8');
            bus.infoPoll = logical(bus.infoPoll);
            if elements > uint32(0)
                bus.infoPoll = logical(in(offset:offset+uint32(elements*1 - 1)));
