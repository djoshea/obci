function [out, valid] = serializeBusWithDataLoggerHeader_ParamBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('ParamBus')
    ...
    outSize = headerLength + getSerializedBusLength_ParamBus(bus, namePrefix);
    assert(outSize <= headerLength + 1440 + 41*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(1983600370), uint16(41), timestamp);
