function [out, valid] = serializeBus_DataLoggerInfoBus(bus, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('DataLoggerInfoBus')
    ...
    valid = uint8(0);
    coder.varsize('out', 206 + 5*numel(namePrefix));
    outSize = getSerializedBusLength_DataLoggerInfoBus(bus, namePrefix);
    assert(outSize <= 206 + 5*numel(namePrefix));
