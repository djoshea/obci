function [bus, valid, offset] = deserializeBus_DisplayInfoRequestBus(input, offset, valid, namePrefix)
%#codegen
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        if valid && elements == uint32(0)
            ...
            if elements > uint32(0)
                bus.infoPoll(1:elements) = typecast(in(offset:offset+uint32(elements*1 - 1))', 'uint8')';
