function [bus, valid, offset] = deserializeBusForMatlab_ParamBus(input, offset, valid, namePrefix)
%#codegen
...
%   BusSerialize.writeDeserializeBusCode('ParamBus')
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        if valid && elements == uint32(0)
            ...
            bus.conditionsToDecode = zeros([4 1], 'uint8');
            bus.conditionsToDecode = logical(bus.conditionsToDecode);
            if elements > uint32(0)
                bus.conditionsToDecode = logical(in(offset:offset+uint32(elements*1 - 1)));
                offset = offset + uint32(elements*1);
            end
        end
    end
