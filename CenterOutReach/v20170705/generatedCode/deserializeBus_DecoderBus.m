function [bus, valid, offset] = deserializeBus_DecoderBus(input, offset, valid, namePrefix)
%#codegen
...
%   BusSerialize.writeDeserializeBusCode('DecoderBus')
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        if valid && elements == uint32(0)
            ...
            if elements > uint32(0)
                bus.classifierInfo(1:elements) = typecast(in(offset:offset+uint32(elements*1 - 1))', 'uint8')';
