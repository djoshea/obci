function [out, valid] = serializeBusWithDataLoggerHeader_ScopeEventBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
% DO NOT EDIT: Auto-generated by 
%   BusSerialize.writeSerializeBusCode('ScopeEventBus')
    ...
    coder.varsize('out', 33 + 1*numel(namePrefix) + headerLength);
    outSize = headerLength + getSerializedBusLength_ScopeEventBus(bus, namePrefix);
    assert(outSize <= headerLength + 33 + 1*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(1151072935), uint16(1), timestamp);
    ...
    % event data type id
    if(offset > numel(out)), return, end
    out(offset) = uint8(8); % data type is ScopeEvent
    offset = offset + uint32(1);
    ...
    out(offset) = uint8(1);
    offset = offset + uint32(1);
    % converting enum type ScopeEvent to string
    coder.varsize('enumAsStr_event', 14);
    enumAsStr_event = zeros(0, 1, 'uint8');
    for iEnum = 1:numel(bus.event)
        enumAsStr_event = [enumAsStr_event; uint8(enumToString_ScopeEvent(bus.event(iEnum)))']; %#ok<AGROW>
