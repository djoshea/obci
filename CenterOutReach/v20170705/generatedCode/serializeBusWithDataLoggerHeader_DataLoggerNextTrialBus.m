function [out, valid] = serializeBusWithDataLoggerHeader_DataLoggerNextTrialBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('DataLoggerNextTrialBus')
    ...
    outSize = headerLength + getSerializedBusLength_DataLoggerNextTrialBus(bus, namePrefix);
    assert(outSize <= headerLength + 23 + 1*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(1404846238), uint16(1), timestamp);
