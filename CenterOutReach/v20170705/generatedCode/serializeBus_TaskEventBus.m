function [out, valid] = serializeBus_TaskEventBus(bus, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('TaskEventBus')
    ...
    valid = uint8(0);
    coder.varsize('out', 46 + 1*numel(namePrefix));
    outSize = getSerializedBusLength_TaskEventBus(bus, namePrefix);
    assert(outSize <= 46 + 1*numel(namePrefix));
