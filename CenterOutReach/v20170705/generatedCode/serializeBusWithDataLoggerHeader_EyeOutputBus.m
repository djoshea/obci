function [out, valid] = serializeBusWithDataLoggerHeader_EyeOutputBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('EyeOutputBus')
    ...
    outSize = headerLength + getSerializedBusLength_EyeOutputBus(bus, namePrefix);
    assert(outSize <= headerLength + 50 + 2*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(2475777960), uint16(2), timestamp);
