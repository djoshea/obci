function [bus, valid, offset] = deserializeBusForMatlab_TaskEventBus(input, offset, valid, namePrefix)
%#codegen
% DO NOT EDIT: Auto-generated by 
%   BusSerialize.writeDeserializeBusCode('TaskEventBus')
    ...
    bus = initializeBus_TaskEventBus();
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        ...
        valid = uint8(0);
        bus.event = repmat(TaskEvent.CenterOnset, [1 1]);
    else
        sz = typecast(in(offset:(offset+uint32(2-1))), 'uint16')';
        offset = offset + uint32(2);
        % check size
        if prod(sz) > uint16(27), valid = uint8(0); end
        elements = uint32(1);
        for i = 1:1
        ...
        if valid && elements == uint32(0)
            ...
            assert(elements <= uint32(27));
            bus.event = repmat(TaskEvent.CenterOnset, [1 1]);
            if elements > uint32(0)
                coder.varsize('tempVar_event', 1);
                tempVar_event = repmat(TaskEvent.CenterOnset, [1 1]); % initialize enum to size with default
                [tempVar_event, nValues, valueValid] = semicolonDelimitedStringToEnumVector_TaskEvent(typecast(in(offset:offset+uint32(elements*1 - 1))', 'uint8')',tempVar_event);
                bus.event = repmat(TaskEvent.CenterOnset, [1 1]);
                if nValues <= 1 && valueValid
                    ...
                    bus.event = typecast(in(offset:offset+uint32(elements*1 - 1)), 'char');
                end
