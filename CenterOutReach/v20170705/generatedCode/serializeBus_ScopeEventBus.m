function [out, valid] = serializeBus_ScopeEventBus(bus, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('ScopeEventBus')
    ...
    valid = uint8(0);
    coder.varsize('out', 33 + 1*numel(namePrefix));
    outSize = getSerializedBusLength_ScopeEventBus(bus, namePrefix);
    assert(outSize <= 33 + 1*numel(namePrefix));
