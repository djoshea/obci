function [bus, valid, offset] = deserializeBusForMatlab_ConditionBus(input, offset, valid, namePrefix)
%#codegen
% DO NOT EDIT: Auto-generated by 
%   BusSerialize.writeDeserializeBusCode('ConditionBus')
    ...
    bus = initializeBus_ConditionBus();
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if valid && elements == uint32(0)
            ...
            if elements > uint32(0)
                ...
                if nValues <= 1 && valueValid
                    ...
                    bus.targetDirectionName = typecast(in(offset:offset+uint32(elements*1 - 1)), 'char');
                end
                offset = offset + uint32(elements*1);
            end
        end
        ...
        if valid && elements == uint32(0)
            ...
            if elements > uint32(0)
                ...
                if nValues <= 1 && valueValid
                    ...
                    % unknown enum value, leaving as char only for Matlab
                    bus.trialCategory = typecast(in(offset:offset+uint32(elements*1 - 1)), 'char');
                end
                offset = offset + uint32(elements*1);
            end
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Deserializing variable-sized field conditionDesc
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    expectedHeader_conditionDesc = uint8([1, 4, typecast(uint16(numel(namePrefix) + 13), 'uint8'), namePrefix, 'conditionDesc', typecast(uint16(4), 'uint8'), 'char', 8, 1])';
    for headerOffset = 1:uint32(25+numel(namePrefix)-1)
        valid = uint8(valid && in(offset+headerOffset-1) == expectedHeader_conditionDesc(headerOffset));
    end
    offset = offset + uint32(25 + numel(namePrefix));

    % Establishing size
    coder.varsize('bus.conditionDesc', 50);
    if valid == uint8(0) || offset + uint32(2 - 1) > numel(in)
        % buffer not large enough for header
        valid = uint8(0);
        bus.conditionDesc = zeros([50 1], 'uint8');
    else
        sz = typecast(in(offset:(offset+uint32(2-1))), 'uint16')';
        offset = offset + uint32(2);
        % check size
        ...
        if valid && elements == uint32(0)
            % assigning empty value
            bus.conditionDesc = zeros([0 1], 'uint8');
        else
            % mollify codegen
            assert(sz(1) <= uint16(50));
            % read and typecast data
            assert(elements <= uint32(50));
            bus.conditionDesc = zeros([sz uint16(1)], 'uint8');
            bus.conditionDesc = char(bus.conditionDesc);
            if isempty(bus.conditionDesc), bus.conditionDesc = ''; end
            if elements > uint32(0)
                bus.conditionDesc = char(in(offset:offset+uint32(elements*1 - 1)));
                if size(bus.conditionDesc, 1) > size(bus.conditionDesc, 2) && size(bus.conditionDesc, 2) == 1, bus.conditionDesc = bus.conditionDesc'; end
                offset = offset + uint32(elements*1);
            end
        end
    end
