function [out, valid] = serializeBusWithDataLoggerHeader_RigOutputBus(bus, groupType, groupName, timestamp, namePrefix)
%#codegen
...
%   BusSerialize.writeSerializeBusCode('RigOutputBus')
    ...
    coder.varsize('out', 172760 + 11*numel(namePrefix) + headerLength);
    outSize = headerLength + getSerializedBusLength_RigOutputBus(bus, namePrefix);
    assert(outSize <= headerLength + 172760 + 11*numel(namePrefix));
    out = zeros(outSize, 1, 'uint8');
    offset = uint32(1);

    % Serialize data logger header
    header = BusSerialize.serializeDataLoggerHeader(groupType, uint8([namePrefixBytes, groupName]), uint32(1618978819), uint16(11), timestamp);
